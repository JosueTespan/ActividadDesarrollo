...
export function NuevaMoto() {
    ...
    const guardarMoto = async (e: FormEvent) => {
        ...
        try {
            ...
            const response = await fetch(`${appsettings.apiUrl}Moto/Nuevo`, {
                method: "POST",
                headers: {
            ...
            navigate(`/nuevoservicio/${data.moto.idCliente}`);
        } catch (error) {
            console.error("Error:", error);
            Swal.fire({
                title: "Error",
                text: error instanceof Error ? error.message : "Error al guardar la motocicleta",
                icon: "error"
            });
        }
    };

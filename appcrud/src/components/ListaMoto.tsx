...
export function ListaMoto() {
    ...
    const obtenerMotos = async () => {
        const response = await fetch(`${appsettings.apiUrl}Moto/Lista`)
        if (response.ok) {
    ...
    const eliminar = (id: number) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const response = await fetch(`${appsettings.apiUrl}Moto/Eliminar/${id}`, { method: "DELETE" })
        ...
        <Container className="mt-5">
            <Row>
                <Col sm={{ size: 10, offset: 1 }}>
                    ...
                    <hr />
                    <Link className="btn btn-success mb-3" to="/nuevamoto/0">Nueva Motocicleta</Link>
